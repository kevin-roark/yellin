...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        if (self.frame.size.height > 500) {
            self.recordButtonAnimUp = 150;
            timeLabelY = 110;
        } else {
            self.recordButtonAnimUp = 140;
            timeLabelY = 60;
        }
        ...
        */
        ...
        self.recordingTimeLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, timeLabelY, self.frame.size.width - 20, 60)];
        self.recordingTimeLabel.textColor = [UIColor blackColor];
        self.recordingTimeLabel.textAlignment = NSTextAlignmentCenter;
        self.recordingTimeLabel.font = [UIFont fontWithName:@"Helvetica-Light" size:40.0];
...
- (void)setToUploading:(BOOL)wantUploading {
    NSLog(@"set to uploading called with bool %hhd", wantUploading);
    if (wantUploading) {
...
- (void)animateRecordButtonUpWithDuration:(CGFloat)duration {
    ...
    [UIView animateWithDuration:duration animations:^{
        // animation block
        self.recordButton.frame = CGRectOffset(self.recordButton.frame, 0, -self.recordButtonAnimUp);
