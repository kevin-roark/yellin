...
- (id)initWithStyle:(UITableViewStyle)style {
    ...
    vc.view.backgroundColor = [UIColor colorWithRed:0.9 green:0.6 blue:0.9 alpha:1];
...
- (IBAction)orignalSoundButtonPressed:(id)sender {
    ...
    PFObject *chirp = self.activeCell.chirp;
    
    if ([[chirp objectId] isEqualToString:[self.playerCache.chirp objectId]]) {
        NSLog(@"same chirp");
        if (!self.playerCache.originalPlayer) {
            [self.activeCell.originalTimeline setLoading:YES];
            [self.playerCache makeOriginalPlayer:^void() {
                [self.activeCell.originalTimeline setLoading:NO];
                [self playOriginalSound];
            }];
        } else {
            [self playOriginalSound];
        }
    } else {
        self.playerCache = [[YellinAudioPlayerCache alloc] initWithChirp:chirp];
        [self.activeCell.originalTimeline setLoading:YES];
        [self.playerCache makeOriginalPlayer:^void() {
            [self.activeCell.originalTimeline setLoading:NO];
            [self playOriginalSound];
...
- (void)playOriginalSound {
    self.audioPlayer = self.playerCache.originalPlayer;
    self.audioPlayer.currentTime = 0;
    self.activeCell.originalSoundLength = self.audioPlayer.duration;
...
- (IBAction)mouthSoundButtonPressed:(id)sender {
    ...
    if ([[chirp objectId] isEqualToString:[self.playerCache.chirp objectId]]) {
        NSLog(@"same chirp");
        if (!self.playerCache.mouthPlayer) {
            [self.activeCell.mouthTimeline setLoading:YES];
            [self.playerCache makeMouthPlayer:^void() {
                [self.activeCell.mouthTimeline setLoading:NO];
                [self playMouthSound];
            }];
        } else {
            [self playMouthSound];
...
- (void)playMouthSound {
    self.audioPlayer = self.playerCache.mouthPlayer;
    self.audioPlayer.currentTime = 0;
    self.activeCell.mouthSoundLength = self.audioPlayer.duration;
