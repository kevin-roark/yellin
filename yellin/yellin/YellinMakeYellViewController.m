...
- (void)recordButtonReleased {
    ...
    self.makeYellView.recordButton.enabled = NO;
    [NSTimer scheduledTimerWithTimeInterval:0.3 target:self selector:@selector(enableRecordButton) userInfo:nil repeats:NO];
...
- (void)enableRecordButton {
    NSLog(@"re enableing the button");
    self.makeYellView.recordButton.enabled = YES;
}
...
- (void)updateMidRecordingStatusView:(NSTimer *)timer {
    if (self.recorder.recording) {
        if (self.recorder.currentTime <= MAX_RECORDING_TIME + 0.04) {
            ...
            [self.recorder stop];
            [self.makeYellView updateRecordingLengthStatus:MAX_RECORDING_TIME];
...
- (void)updateMidPlayingStatusView:(NSTimer *)timer {
    if (self.player.playing) {
        ...
        [timer invalidate];
        timer = nil;
        [self.makeYellView updateRecordingLengthStatus:self.player.duration];
    }
}
